        ...
        if call.data == 'cheque':
            if func.total_cheque(id)[1] >= 8:
                if int(func.total_cheque(id)[0]) < 5000:
                    ...
                    bot.send_message(id,f'Тк сумма заказа меньше 5к, придется доплатить селлеру за доставку :(\n\nСумма заказа: {func.total_cheque(id)[0]}\n'+func.include(id)+'\nПРОВЕРЬ АДРЕС И КОНТАКТ:\n'+func.get_adr(id), reply_markup=kb.input_money, parse_mode='HTML')
            else:
                bot.send_message(id, f'Сумма заказа:{func.total_cheque(id)[0]}\n' + func.include(id)+'<strong>Заказ от восьми товаров!</strong>', reply_markup=kb.board2, parse_mode='HTML')
            ...
            if func.check_payment(id) != None:
               ...
               if int(func.total_cheque(id)[1])<5000:
                    bot.send_message(id,'Оплата принята! Тк у вас заказ меньше чем 5000р, придется доплатить селлеру за доставку: @darknethero')
                    func.pay_seller(sum)
